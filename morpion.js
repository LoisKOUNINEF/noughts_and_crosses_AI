    		...
    		grid.forEach((line, y) => {
    			line.forEach((cell, x) => {
    				if (!cell) {
    					...
    					let score = this.minMax(grid, depth + 1, false);
    					grid[y][x] = null;
    		...
    		grid.forEach((line, y) => {
    			line.forEach((cell, x) => {
    				if (!cell) {
    					...
    					let score = this.minMax(grid, depth + 1, true);
    					grid[y][x] = null;
    ...
    doPlayIa = () => {
    	...
    	grid.forEach((line, y) => {
    		line.forEach((cell, x) => {
    			if (!cell) {
    				grid[y][x] = this.iaPlayer;
    				let score = this.minMax(grid, 0, false);
    				grid[y][x] = null;
    	...
    	this.drawHit(aiMove.x, aiMove.y, this.iaPlayer);
    }
}
